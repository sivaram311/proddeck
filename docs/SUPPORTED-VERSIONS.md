...
| **agentverse-upgrade** | **0.3.1** | **0.3.1** | 4312 / 5312 | **Dispatch peer SoT** · upgrade-staging / upgrade hosts |
| **AgentVerse classic** | **0.3.17** | **0.3.17** | 4310 / 5310 | **Rollback only** — densify + `/desk`; **not** Dispatch default |
| **AgentVerse v2** | side fleet | side fleet | 4311 / 5311 | Industrial / independent; do not disturb |
| **CSS (prod IdP)** | shared | shared | 5900 · css.delena.buzz | Issuer bake · `v0.1.0` |
...
Wave A features: activity-log Lead drain · ports stop dry-run · Device Lab E2E.

| Depends on | Minimum | Why |
|------------|---------|-----|
| CSS | `https://css.delena.buzz` · `clientId=proddeck` · tag `v0.1.0` | Login + catalog JWT |
| Agent Portal | **≥ 0.1.8** | `OS_EVENTS_FORWARD=1` |
...
| Agent Portal | **≥ 0.1.8** | Desk / cancel / quests |
...
| Provides | Consumers |
|----------|-----------|
| `POST /api/os-events` | ProdDeck ≥ 0.6.2 |
